...
- 本机为 **Mainframe-0**，是唯一拥有完整 KSP 1.12.3 TestRun 安装（50+ mod）的工作站。GameData 路径为 `d:\KSP\KSP_1.12.3\TestRun\GameData\`，不应在其他机器上硬编码此路径
...
├── memory/                   # 项目记忆文件 (git 管理)
│   ├── MEMORY.md             # 记忆索引
│   ├── mainframe-0.md        # Mainframe-0 机器信息与路径
│   └── *.md                  # 各主题记忆
